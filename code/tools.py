...
def dset_sanitize_and_filter(dset, return_index=True):
    ...
    for i in tqdm(ids):
        ...
        dset.targets.data.loc[i,"good"] = dset.targets.data.loc[i,"keep"] \
...
def X0X1C_to_MbX1C(values, cov, M0=10.501612):
    ...
    new_covs = {i: jnp.matmul(jnp.matmul(jnp.array([[-2.5/(np.log(10)*values["x0"].loc[i]),0,0], [0,1,0], [0,0,1]]), cov[i]),
                              jnp.array([[-2.5/(np.log(10)*values["x0"].loc[i]),0,0], [0,1,0], [0,0,1]])) for i in values.index}
    return new_values, new_covs


def sncosmo_to_edris(res, data, index, interest_vars=["x1", "c"], n_bins=10, M0=10.501612):
    n = len(index)
    
    covariances = {i: np.array(res.loc[i].loc[["x0","x1","c"]][["cov_x0", "cov_x1", "cov_c"]]) for i in index}
    
    stacked_res = pandas.DataFrame({**{col: np.array(res["value"].loc[map(lambda x: (x, col), index)]) for col in ["x0","x1","c"]}}, index=index)
    ...
    full_cov_sorted = full_matrix[:,[*[k*i for i in range(n)], *[k*i+1 for i in range(n)], *[k*i+2 for i in range(n)]]]\
                                 [[*[k*i for i in range(n)], *[k*i+1 for i in range(n)], *[k*i+2 for i in range(n)]],:]
    
    cov = FullCovariance(full_cov_sorted[:n,:n], full_cov_sorted[n:,n:], full_cov_sorted[:n,n:])
    var = jnp.array([values[col].to_list() for col in interest_vars]).flatten()
    mag = jnp.array(values['Mb'].to_list())
    obs = Obs(mag, var)
    
    exp= {'z': jnp.array(data.loc[index]["z"].to_list()),
...
def get_cov_edris(hess, invcov=False):
    ...
    row1 = jnp.hstack((hess['coef']['coef'], hess['coef']['mu_bins'], hess['coef']['variables'].reshape(n_var,n_var*n)))
    row2 = jnp.hstack((hess['mu_bins']['coef'], hess['mu_bins']['mu_bins'], hess['mu_bins']['variables'].reshape(n_bins,n_var*n)))
    row3 = jnp.hstack((hess['variables']['coef'].reshape(n_var*n,n_var),
    ...
    if invcov:
        return .5 * flatten_hessian
    return jnp.linalg.inv(.5*flatten_hessian)
